...
function getNovedades() {
  ...
  return data.slice(1).map(function(row, i) {
    return {
      rowIndex: i + 2,
      id: (row[0] || '').toString(),
      fecha: row[1] instanceof Date ? Utilities.formatDate(row[1], Session.getScriptTimeZone(), "dd/MM/yyyy") : (row[1] || '').toString(),
      hora: row[2] instanceof Date ? Utilities.formatDate(row[2], Session.getScriptTimeZone(), "HH:mm") : (row[2] || '').toString(),
...
function getSolicitudes() {
  ...
  return data.slice(1).map(function(row, i) {
    return {
      ...
      fechaDesde: row[5] instanceof Date ? Utilities.formatDate(row[5], Session.getScriptTimeZone(), "dd/MM/yyyy") : (row[5] || '').toString(),
      fechaHasta: row[6] instanceof Date ? Utilities.formatDate(row[6], Session.getScriptTimeZone(), "dd/MM/yyyy") : (row[6] || '').toString(),
      horaDesde: row[7] instanceof Date ? Utilities.formatDate(row[7], Session.getScriptTimeZone(), "HH:mm") : (row[7] || '').toString(),
